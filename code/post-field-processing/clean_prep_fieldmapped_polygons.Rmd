...
today <- "1_17_2025" #a string containing the date of run
...
#This function takes in a set of polygons and returns the same set of polygons with the area of the polygon added as a column called [nm], in whatever units the CRS is in
st.area.to.poly <- function(polys, nm) {
  out <- polys |>
...
aopPolyCleanHalf <- aopPolyClean %>%
  split(seq_len(nrow(.))) |>
  purrr::map(buffer.set.to.half.diam, 0.01) |>
  dplyr::bind_rows()
...
```{r}
#Write out
sf::st_write(uasPolyClean, here::here(datDerDir, paste0("uas_polygons_", today, "_analysis_ready.gpkg")), append = FALSE)
sf::st_write(uasPolyCleanHalf, here::here(datDerDir, paste0("uas_polygons_", today, "_analysis_ready_half_diam.gpkg")), append = FALSE)

#Write out
sf::st_write(aopPolyClean, here::here(datDerDir, paste0("aop_polygons_", today, "_analysis_ready.gpkg")), append = FALSE)
sf::st_write(aopPolyCleanHalf, here::here(datDerDir, paste0("aop_polygons_", today, "_analysis_ready_half_diam.gpkg")), append = FALSE)
